    ...
    private List<SniperProxy> listProxies(String url) {
        ...
        try {
            ...
            for (String data : dataList) {
                data = data.trim();
                if (data.contains(":")) {
                    if (data.startsWith("socks")) {
                        String info = data.split("//")[1];
                        String username = null;
                        String password = null;
                        
                        if (info.contains("@")) {
                            String[] proxyParts = info.split("@");
                        ...
                        String[] proxyData = info.split(":");

                        proxies.add(new SniperProxy(proxyData[0], Integer.parseInt(proxyData[1]), username, password, true));
                    } else {
                        String username = null;
                        String password = null;
